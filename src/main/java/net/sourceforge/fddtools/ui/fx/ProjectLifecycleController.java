...
public class ProjectLifecycleController {
    ...
    // --- Implementation ---
    private void createFreshProject(){ FDDINode rootNode = createNewRootNode(); host.rebuildProjectUI(rootNode, true); LOGGER.info("New project created"); }
    ...
    private void openProjectDialog(){ try { FileChooser fc = new FileChooser(); fc.setTitle("Open FDD Project"); fc.getExtensionFilters().addAll(new FileChooser.ExtensionFilter("FDD Files","*.fddi","*.xml"), new FileChooser.ExtensionFilter("All Files","*.*")); File f = fc.showOpenDialog(host.getPrimaryStage()); if(f!=null){ host.rebuildProjectUI((FDDINode)FDDIXMLFileReader.read(f.getAbsolutePath()), false); RecentFilesService.getInstance().addRecentFile(f.getAbsolutePath()); host.refreshRecentFilesMenu(); PreferencesService.getInstance().setLastProjectPath(f.getAbsolutePath()); PreferencesService.getInstance().flushNow(); } } catch(Exception e){ LOGGER.error("Open failed: {}", e.getMessage(), e); host.showErrorDialog("Open Project Failed", "Error loading file: "+e.getMessage()); } }

    private void openRecentInternal(String path){ File f = new File(path); if(!f.exists()){ host.showErrorDialog("Open Recent", "File no longer exists: "+path); RecentFilesService.getInstance().clear(); host.refreshRecentFilesMenu(); return; } try { host.rebuildProjectUI((FDDINode)FDDIXMLFileReader.read(f.getAbsolutePath()), false); RecentFilesService.getInstance().addRecentFile(path); host.refreshRecentFilesMenu(); } catch(Exception e){ LOGGER.error("Failed to load recent: {}", e.getMessage(), e); host.showErrorDialog("Open Project Failed","Error loading file: "+e.getMessage()); } }

    private void handleUnsavedChanges(Runnable proceed){ UnsavedChangesHandler.handle(ModelState.getInstance().isDirty(), () -> { ButtonType save = new ButtonType("Save"); ButtonType dont = new ButtonType("Don't Save"); ButtonType cancel = new ButtonType("Cancel", ButtonBar.ButtonData.CANCEL_CLOSE); ButtonType c = DialogService.getInstance().confirmWithChoices(host.getPrimaryStage(),"Unsaved Changes","Save changes before continuing?","You have unsaved changes.", save,dont,cancel); if(c==save) return UnsavedChangesHandler.Decision.SAVE; if(c==dont) return UnsavedChangesHandler.Decision.DONT_SAVE; return UnsavedChangesHandler.Decision.CANCEL; }, this::saveCurrentProjectBlocking, proceed); }
