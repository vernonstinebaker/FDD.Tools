...
public class ProjectLifecycleController {
    ...
    private void openProjectDialog(){ try { FileChooser fc = new FileChooser(); fc.setTitle("Open FDD Project"); fc.getExtensionFilters().addAll(new FileChooser.ExtensionFilter("FDD Files","*.fddi","*.xml"), new FileChooser.ExtensionFilter("All Files","*.*")); File f = fc.showOpenDialog(host.getPrimaryStage()); if(f!=null){ FDDINode root=(FDDINode)FDDIXMLFileReader.read(f.getAbsolutePath()); host.rebuildProjectUI(root, false); net.sourceforge.fddtools.state.ModelEventBus.get().publish(net.sourceforge.fddtools.state.ModelEventBus.EventType.PROJECT_LOADED, root); PreferencesService.getInstance().addRecentFile(f.getAbsolutePath()); host.refreshRecentFilesMenu(); PreferencesService.getInstance().setLastProjectPath(f.getAbsolutePath()); PreferencesService.getInstance().flushNow(); } } catch(Exception e){ LOGGER.error("Open failed: {}", e.getMessage(), e); host.showErrorDialog("Open Project Failed", "Error loading file: "+e.getMessage()); } }

    private void openRecentInternal(String path){ File f = new File(path); if(!f.exists()){ host.showErrorDialog("Open Recent", "File no longer exists: "+path); PreferencesService.getInstance().clearRecentFiles(); host.refreshRecentFilesMenu(); return; } try { FDDINode root=(FDDINode)FDDIXMLFileReader.read(f.getAbsolutePath()); host.rebuildProjectUI(root, false); net.sourceforge.fddtools.state.ModelEventBus.get().publish(net.sourceforge.fddtools.state.ModelEventBus.EventType.PROJECT_LOADED, root); PreferencesService.getInstance().addRecentFile(path); host.refreshRecentFilesMenu(); } catch(Exception e){ LOGGER.error("Failed to load recent: {}", e.getMessage(), e); host.showErrorDialog("Open Project Failed","Error loading file: "+e.getMessage()); } }
    ...
    private boolean saveCurrentProjectBlocking(){ try{ ProjectService ps = ProjectService.getInstance(); if(ps.getRoot()==null) return true; String before = ps.getAbsolutePath(); if(before!=null){ boolean ok=ps.save(); if(ok){ PreferencesService.getInstance().setLastProjectPath(ps.getAbsolutePath()); PreferencesService.getInstance().flushNow(); host.updateTitle(); } return ok; } else { FileChooser fc=new FileChooser(); fc.setTitle("Save FDD Project"); fc.getExtensionFilters().addAll(new FileChooser.ExtensionFilter("FDD Files","*.fddi"), new FileChooser.ExtensionFilter("XML Files","*.xml")); fc.setInitialFileName(FileNameUtil.buildDefaultSaveFileName(ps.getDisplayName())); File f=fc.showSaveDialog(host.getPrimaryStage()); if(f==null) return false; boolean ok=ps.saveAs(FileNameUtil.ensureFddiOrXmlExtension(f.getAbsolutePath())); if(ok){ PreferencesService.getInstance().setLastProjectPath(ps.getAbsolutePath()); PreferencesService.getInstance().flushNow(); host.updateTitle(); } return ok; } } catch(Exception ex){ LOGGER.error("Save failed: {}", ex.getMessage(), ex); host.showErrorDialog("Save Failed", ex.getMessage()); return false; } }
}
